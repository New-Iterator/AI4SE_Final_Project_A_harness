...
- **GitHub Actions**：`test` + `docker-build` 两个 job，每次 push 构建 Docker 镜像，tag 时自动推送至 GitHub Container Registry
- **GitLab CI**：`unit-test` + `build` + `docker-build` 三个 stage，使用 node:20 镜像，94 测试全部通过
